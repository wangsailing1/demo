...
class ServerSuperPlayer(ModelBase):
    ...
    def __init__(self, uid=None):
        ...
        super(ServerSuperPlayer, self).__init__(self.uid)

    @classmethod
    def get(cls, uid, server_name='', **kwargs):
        o = super(ServerSuperPlayer, cls).get(uid, server_name=server_name, **kwargs)
        o.check_version()
        return o
    ...
    def send_bag(self, tp='spend'):
        ...
        if can_send_times and configs:
            ...
            send_rank = ServerSuperPlayerRank(self.version)
            for i in range(can_send_times):
                ...
                if tp == 'spend':
                    ...
                    self.day_pay_send += 1
                self.day_send_bag += 1
                redbag = ServerRedBag(self.version)
    ...
    def add_send_notice(self):
        ...
        msg = i18n_msg.get(1208, lan) % (server_name, self.mm.user.name)
        data_ = {
            'msg': msg,
            'notice_lv': 6,
        }
...
class ServerRedBag(ModelTools):
    ...
    def __init__(self, version):
        ...
        红包类
        """
        super(ServerRedBag, self).__init__()
        self.version = version
...
class ServerSuperPlayerShop(ModelBase):
    SERVER_NAME = 'master'
    ...
    def __init__(self, version=None):
        ...
        super(ServerSuperPlayerShop, self).__init__(self.version)
...
class ServerSuperPlayerRank(ModelTools):
...
ModelManager.register_model('serversuperplayer', ServerSuperPlayer)