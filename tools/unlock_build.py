...
# ACHIVE_SORT = 2  # 业绩目标
# SIGN_SORT = 3  # 每月签到
CEREMONY_SORT = 4  # 颁奖典礼
# CONTINUE_SORT = 5  # 持续运营
# MAIL_SORT = 6  # 邮件
# PHONE_SORT = 7  # 手机信息
# APPLY_FRIEND = 8  # 合作申请
FILMING_SORT = 9  # 自制影视
SCRIPT_GACHA_SORT = 10  # 编剧之家
# QUALITY_SORT = 11  # 格调
# CARD_LOVE = 12  # 羁绊
# ACTOR_CHAT = 13  # 问候
# RAPPORT = 14  # 约会
# # GUILD_FIGHT_SORT = 15  # 旅行
# CARD_BOOK = 16  # 组合（艺人）
# SCRIPT_SORT = 17  # 剧本
# SCRIPT_GROUP_SORT = 18  # 剧本（组合）
GACHA_SORT = 19  # 星探之屋
BLOCK_RANKING = 24  # 世界巡回赛
RANKING = 25  # 全服金榜
...
def check_build(mm, build_unlock_id):
    ...
    :return:
    """
    building_unlock_config = game_config.homepage_button.get(build_unlock_id)
    ...
    for unlock_type in unlock_build_type.values():
        ...
        for unlock_id, unlock_config in unlock_mapping.iteritems():
            ...
            if unlock_id not in mm.user.unlock_build:
                ...
                #     mm.user.unlock_build.append(unlock_id)
                #     is_save = True
                if mm.user.level >= unlock_limit and unlock_id not in mm.user.unlock_build:
                    mm.user.unlock_build.append(unlock_id)
                    is_save = True
