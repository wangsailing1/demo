...
# 参数化构建过程
    # Dynamic Choice Parameter
        # Name: tag
        # Choice Script: def ver_keys = [ 'bash', '-c', 'cd /var/lib/jenkins/jobs/genesis2_dev_res/workspace; git pull>/dev/null; git fetch --all>/dev/null; git tag -l "t*"|sort -r|head -10' ]
...
DEST_RES_DIR="/data/sites/genesis2_dev/logs/lr"
DEST_VER_DIR="/data/sites/genesis2_dev/logs/client_resource"
...
do
    ...
    if [ "x$i" == "x$END_TAG" ]; then
        ...
        echo $json_str > $RES_DIR/$i.json
        continue
    fi

    FILE_LIST=$(git diff --name-only ${i} ${END_TAG})

    git archive -o $RES_DIR/${i}.zip $END_TAG $FILE_LIST

    echo $FILE_LIST >> $RES_DIR/$i.txt
    sed -i 's/ /\n/g' $RES_DIR/$i.txt

    md5_value=$(gerenate_md5 "$i")
