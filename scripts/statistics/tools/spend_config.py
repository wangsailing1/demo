...
if len(sys.argv) == 2:
    ...
    print 'mysql_host=%s; mysql_passwd=%s; mysql_db=%s; ' \
          'mysql_user=%s; mysql_tprefix=%s' % (settings.SPEND_CONFIG['host'], settings.SPEND_CONFIG['passwd'],
                                               settings.SPEND_CONFIG['db'], settings.SPEND_CONFIG['user'],
                                               settings.SPEND_CONFIG['table_prefix'])
else:
