...
from scripts.statistics.tools import reg_user
...
def main():
    ...
    file_name = 'reg_%s' % date_stamp
    open('%s/%s' % (real_path, file_name), 'w')     # 没有新增时需要生成空文件

    reg_uids = reg_user.get_reg_all_user(today=today, withscores=True)

    for data in reg_uids:
        x_uid = data[0]
        x_reg_time = int(data[1])
        x_date = datetime.datetime.fromtimestamp(x_reg_time).strftime('%Y-%m-%d')
        x_time = datetime.datetime.fromtimestamp(x_reg_time).strftime('%H:%M:%S')
