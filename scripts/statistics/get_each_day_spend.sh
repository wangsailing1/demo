...
params=$(python $filepath/tools/spend_config.py $env | tail -n1)
eval $params;
...
do
    ...
    echo "$yesterday<=subtime<=$today";
    for j in {0..15};
    do
        mysql -h$mysql_host -u$mysql_user -p$mysql_passwd $mysql_db -N -e"select * from ${mysql_tprefix}_${j} where
        subtime >= '$yesterday' and subtime <= '$today' " >> $filepath/../../statistic/$env/spendlog_$hjq_subfix;
    done
