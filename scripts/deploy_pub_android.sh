...
GIT_REPO_PATH="/data/release/superHero2/repo/backend"
...
/usr/bin/rsync --dry-run -aI --progress --recursive --exclude='upload_xls' --exclude='.git' --exclude='logs' --exclude='test/local_config.py' $temp_dir "admin@$THOST:/data/sites/superhero2_backend/"
...
do
    echo $host
    /usr/bin/rsync -aI --progress --recursive --exclude='upload_xls' --exclude='.git' --exclude='logs' --exclude='test/local_config.py' $temp_dir "admin@${host}:/data/sites/superhero2_backend/"
