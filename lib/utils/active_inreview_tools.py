...
def get_active_inreview_start_end_time(config, server=None, channel_id=None, config_type=None, diff_time=0):
    ...
    for active_id, value in config.iteritems():
        ...
        if no_server_id:
            if (server is not None and channel_id is not None) and ((server in no_server_id) or (open_channel_id and channel_id not in open_channel_id)):
                # return 0, 0, 0
                continue
        else:
            if (channel_id is not None and config_type is not None) and ((open_server_type and config_type not in open_server_type) or (open_channel_id and channel_id not in open_channel_id)):
                # return 0, 0, 0
                continue
...
def get_active_inreview_version_start_end_time(config):
    """
    # 获取活动的开启和结束的时间
    :param config: 配置表 如game_config.roulette
    :return: 版本号,开始时间,结束时间
...
def active_inreview_version(config, diff_time=0, server_id=''):
    """
    # 获取活动的版本号
    :param config: 配置表 如game_config.roulette
    :return: 版本号
